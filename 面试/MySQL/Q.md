...
# 引入MVCC有什么用
- 在没有 MVCC 的情况下，为了保证事务的隔离性，数据库通常会使用 **锁 (Locking)** 来控制并发访问。比如，一个事务在读取数据时加读锁，在修改数据时加写锁。
    - **问题：** 锁虽然能保证隔离性，但会严重影响并发性能。如果一个长时间运行的读事务锁住了数据，其他写事务就必须等待；反之，一个写事务锁住了数据，其他读写事务也必须等待。在高并发场景下，这会导致大量的锁等待，降低系统的吞吐量。
    - **需求：** 我们希望在保证事务隔离性的同时，尽量提高数据库的并发处理能力，尤其是实现 **读不阻塞写，写不阻塞读** 的效果。
    MVCC 正是为了解决这个问题而诞生的。它是一种 **乐观并发控制** 的思想，通过保存数据的多个版本，==使得读操作可以读取数据的旧版本，而无需等待写操作释放锁==。