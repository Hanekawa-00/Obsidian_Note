...
我们当前播放声音是直接通过解析URL形式播放的，这种播放不安全，但是免费。正确的播放应该是文件的Id+播放凭证，但是，这种播放方式在小程序中是收费的。所以我们暂时是使用的解析URL形式播放的。下面就来实现一个以文件Id+播放凭证的方式进行播放。

在TrackInfoApiController控制器 添加方法
...
public interface UserListenProcessFeignClient {
    ...
    Result<BigDecimal> getTrackBreakSecond(@PathVariable Long trackId);
}
```
...
```java
/**
  * 文档：https://cloud.tencent.com/document/product/266/46217
  * 实例代码：https://cloud.tencent.com/document/product/266/42437
  * @param mediaFileId
  * @return
  */
@Override
